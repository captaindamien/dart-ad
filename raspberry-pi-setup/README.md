...
Выход из рекламы — только по шапке меню (набор эталонов `public/exit/`),
принудительного таймаута нет: так решил
...
- **Через `STUCK_WARN_SEC` (300 с) в heartbeat уходит `fault=ad_stuck`** с
  диагностикой: сколько секунд без шапки, максимальный отклик каждого эталона
  выхода за сеанс и сколько раз повторился marker1. Дэшборд показывает это как
  предупреждение, состояние остаётся `playing` — реклама действительно идёт.
  По откликам сразу видно, в чём дело: `0.6–0.74` у какого-то эталона — порог
  или кадрирование, всё `~0.2` — меню на экране не появляется вовсе (никто не
  играет, аттракт крутится по кругу — это штатно).
...
### 6. Набор эталонов выхода (public/exit/)

Шапка меню автомата меняет цвет и состав по режиму: красная 701, синяя
Cricket, зелёная Count-Up, розовая party, оранжевая Medley, бирюзовая Online
Match, тёмно-синяя League, зелёная Player Select, и серые шапки экранов
CARD DATA / SETTINGS / MY VIDEO UPLOAD. Плюс модели различаются: у
DARTSLIVE 3 LITE (ilsport3) активный пункт шапки увеличен, и эталон с
DARTSLIVE 3 (бывший `marker2.png`, теперь `exit/game_select_ds3.png`) на нём
давал 0.73 при пороге 0.75. С 1.4.0 выход из рекламы — по **любому** PNG из
`public/exit/`; агент пишет в лог, какой сработал:

```
[MARKERS] вход=marker.png; выход (9): card_data_lite 480x38, game_select_ds3 480x45, …
[DETECT] выход из рекламы по exit/game_select_lite: primary=0.987
```

Как считается: основной детектор (`matchTemplate`, порог `THRESHOLD`) только
в верхней доле кадра (`EXIT_SEARCH_BAND`, 0.3), лучший из набора. Терпимый к
масштабу детектор для эталонов выхода **выключен** (`DETECT_TOLERANT_EXIT=0`):
на записи с ilsport3 (729 кадров без шапки) основной не поднимался выше 0.28,
а терпимый на экране WARNING и слайдах аттракта давал до 0.80 — это ложный
выход. Для marker1 терпимый работает по-прежнему.

Замер по записи ilsport3 (14.09.2026): один эталон синей шапки покрывает
красную, синюю, зелёную и розовую на 0.99–1.00; остальные режимы — по
своему эталону, на своих кадрах 0.90–1.00. Эталоны — `*_lite.png` — сняты с
LITE; на DARTSLIVE 3 нужен такой же прогон записи, и по нему добавятся
`*_ds3.png` для остальных режимов.

**Добавить режим**: записать экран (раздел 7), вырезать полосу шапки
`tools/make_marker.py <снимок> --y0 8 --y1 160 --out public/exit/<имя>.png`,
проверить `tools/rec_report.py` (на своих ≥ 0.85, на чужих ≤ 0.6, `--replay`
без лишних переходов), запушить. Кода это не касается.

### 7. Запись экрана для подбора маркеров

Эталоны выхода (раздел 6) режутся из кадров самого автомата, а снаружи
карту захвата не открыть — поэтому агент с 1.3.1 умеет записывать экран сам. Снаружи (`ffmpeg`)
...
| `detect.csv` | на каждый кадр: отклики marker1 и лучшего эталона выхода с его именем, fps, состояние, номер кадра видео, имя снимка |
...
```bash
python3 tools/rec_report.py ~/rec/<дата-время>            # контактные листы shots_sheet_N.jpg + отчёт по public/exit/*
python3 tools/make_marker.py ~/rec/<дата-время>/shots/0012_t0084.5_video.png --y0 8 --y1 160 --out cand/new_mode.png
mkdir -p ~/rec/<дата-время>/labels/new_mode && cp ~/rec/<дата-время>/shots/0012_*.png ~/rec/<дата-время>/labels/new_mode/
python3 tools/rec_report.py ~/rec/<дата-время> --templates public/exit/*.png cand/new_mode.png --replay
```
...
Полоса `--y0 8 --y1 160` — внутри шапки: ниже 160 px на части экранов уже
начинается содержимое, и эталон с ним перестаёт узнавать ту же шапку на
другом экране.
