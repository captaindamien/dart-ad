...
Зачем. Выход из рекламы ловится по эталону шапки меню (marker2.png), а в части
режимов автомата шапка другая — и такой режим реклама не отпускает до конца
игры. Чтобы собрать шапки всех режимов, нужен кадр с автомата в каждом из
них. Снаружи карту захвата не открыть: агент держит /dev/videoN монопольно,
...
Что пишется в REC_DIR/<YYYYmmdd-HHMMSS>/:
  ...
  detect.csv   — на каждый обработанный кадр: отклики обоих маркеров, fps,
                 состояние, номер кадра видео и имя снимка.
